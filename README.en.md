...
[Upstream project](https://github.com/GoogleChromeLabs/squoosh) · [简体中文](./README.md)
...
Images are processed in the current browser. This workbench does not upload originals or converted files. The theme preference is saved locally in the browser; reloading or closing the page clears the image queue, so download any results you need to retain. This statement covers the workbench itself; analytics and other external resources on a future host site must be reviewed separately.
...
CI in this source repository installs dependencies and validates the build on Linux and Windows; it does not deploy. This modified version has not been deployed to a production site. Publishing to `play.wangyifang.com` is a separate step: copy `dist/` into `squoosh/` in that site's repository, then let the site's GitHub `main` branch trigger Cloudflare Workers Builds.
