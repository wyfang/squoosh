...
- 选择 PNG 输出时显示复制按钮，可直接将当前 PNG 结果写入剪贴板；其他输出格式不显示复制按钮，也不会为复制而转码。
...
当前支持静态 PNG、JPEG、WebP、AVIF、BMP 输入，不支持动画。一次最多加入 30 张图片，输入总量不超过 100 MB；单张不超过 40 MB、1 亿像素，任一边不超过 16383 像素。大图能否成功处理仍取决于浏览器和设备的可用内存，AVIF 编码通常耗时更长。

复制图片使用浏览器剪贴板能力，需要支持图片写入的浏览器及 HTTPS 或 localhost 环境；能否粘贴取决于目标应用。系统或目标应用可能重写剪贴板图片，粘贴后的文件字节和体积不保证与 PNG 下载结果相同。
